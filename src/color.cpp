#include "main.h"

const color_t COLOR_RED = { 236, 100, 75 };
const color_t COLOR_GREEN = { 135, 211, 124 };
const color_t COLOR_BLACK = { 52, 73, 94 };
const color_t COLOR_BACKGROUND = { 242, 241, 239 };
const color_t COLOR_BLUE = { 0, 138, 230 };
const color_t COLOR_CUBE1 = { 204, 255, 51 };
const color_t COLOR_CUBE2 = { 255, 80, 80 };
const color_t COLOR_CUBE3 = { 0, 102, 0 };
const color_t COLOR_CUBE4 = { 0, 51, 102 };
const color_t COLOR_CUBE5 = { 172, 0, 230 };
const color_t COLOR_CUBE6 = { 204, 68, 0 };
const color_t COLOR_CUBE7 = { 0, 153, 204 };
const color_t COLOR_CUBE8 = { 51, 153, 102 };
const color_t COLOR_CUBE9 = { 135, 211, 124 };
const color_t COLOR_BLAST = { 255, 12, 1 };
const color_t COLOR_BROWN = { 102, 34, 0 };
